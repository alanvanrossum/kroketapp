...
/**
 * This service is responsible for registering players by sending information to
 * the server, and for receiving and acting on messaged sent by the server.
 */
public class ConnectionService extends Service {
   ...
   * This method registers the player by sending the player's name and the type
   * to the server.
   *
   * @param intent
   *          The Intent supplied to
   *          {@link android.content.Context#startService}, as given. This may
   *          be null if the service is being restarted after its process has
   ...
   * @return The return value indicates what semantics the system should use for
   *         the service's current started state. It may be one of the constants
  ...
  private boolean inWaitingActivity() {
    String currentActivity = ((ActivityManager) this.getApplicationContext())
        .getCurrentActivity().getComponentName().getClassName();
    String waitingActivity = WaitingActivity.class.getName();
    String waitingGame = D_Gyroscope.class.getName();
    return (currentActivity.equals(waitingActivity) || currentActivity.equals(waitingGame));
  }
  ...
  /**
   * Sends a message to the server that minigame B is solved and needs a
   * verification.
   */
  public void verifyB() {
  ...
  private class connectTask extends AsyncTask<String, String, GameClient> {
    ...
    /**
     * Returns the class of the minigame that should be started corresponding to
     * the action.
     *
