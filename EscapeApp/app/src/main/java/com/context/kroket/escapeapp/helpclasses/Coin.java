...
/**
 * Abstact superclass for all the different coin classes.
 */
public abstract class Coin {
   ...
   *
   * @param gyroX
   *          The location of the skullball on the x-axis
   * @param gyroY
   *          The location of the skullball on the y-axis
   * @return True if the skullball overlaps with the coin False if it doesn't.
   */
  ...
  /**
   * This method is called for each coin when a coin is collected. This method
   * places the coin on a random location on the screen. This method makes sure
   * the coin does not initially overlap with the skullball
   *
   * @param rand
   *          A Java Math.Random object
   * @param width
   *          The width of the screen minus three times the width of the
   *          skullball
   * @param height
   *          The height of the screen minus three times the height of the
   *          skullball
   * @param gyroX
   *          The location of the skullball on the x-axis
   * @param gyroY
   *          The location of the skullball on the y-axis
   */
  public void placeRandomly(Random rand, int width, int height, float gyroX, float gyroY) {
    setX(clamp(rand.nextInt(width), gyroX, 70));
    setY(clamp(rand.nextInt(height), gyroY, 70));
  }

  /**
   * This method is called to make sure the coins do not overlap with the
   * skullball.
   *
   * @param value
   *          The random x or y coordinate initally generated in placeRandomly()
   * @param gyroLoc
   *          The x respectively y coordinate containing the location of the
   *          skullball
   * @param size
